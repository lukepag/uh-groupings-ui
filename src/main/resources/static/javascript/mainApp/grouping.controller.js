...
(() => {
    ...
    function GroupingJsController($scope, $controller, $window, $uibModal, groupingsService, PAGE_SIZE, Message, Threshold, Utility) {
        ...
        $scope.addMembers = (listName, uhIdentifiers) => {
            ...
            // IMPORTANT this counts one grouping owner as one owner. This is an issue when counting the amount of owners being added
            ...
            if (listName !== "owner group path") {
                ...
                // below logic is only for adding one group path. eventually add logic for multiAdd
                $scope.waitingForImportResponse = false; // Small spinner off
        ...
        const handleSuccessfulAdd = (res) => {
            ...
            if ($scope.isBatchImport) {
                ...
                // RIGHT NOW, it always defaults back to the normal modal ID. should i put this in addOnClick instead and initialize it as null?
                $scope.addModalId = "add-modal";
                $scope.addModalURL = "modal/addModal";
                $scope.displayDynamicModal(
            ...
            $scope.dynamicModal.result.finally(() => {
                ...
                if ($scope.listName === "admins") {
                    ...
                    // here is where it starts to retrieve the grouping members
                    $scope.getGroupingInformation();
                    $scope.syncDestArray = [];
                }
            });
        ...
        $scope.displayAddModal = (options) => {
            ...
            console.log("Modal id: " + $scope.addModalId);
            console.log("Modal url: " + $scope.addModalURL);
         ...
         *
         * why isnt the addMembers and removeMembers function consistent with each other?
         *
         */
        $scope.removeMembers = (listName) => {
            ...
            // Sanitize only if the input is a uhIdentifier
...
/*            if (!$scope.fetchMemberProperties(uhIdentifiers, listName)) {
            ...
            // Why is the conditional an ===? shouldnt it be >=
            // Still have to add a removeError modal for group paths, but this remove error modal seems to only display when there are more owners to remove then there are owners
            ...
            // theres no extra is empty check after the filter as there is in addMember
        ...
        $scope.displayRemoveModal = (options) => {
            ...
            if ($scope.listName === "owners") {
                // would i need to do this for every single member?
            ...
            console.log("Modal id: " + $scope.removeModalId);
            console.log("Modal url: " + $scope.removeModalURL);
